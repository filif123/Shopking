...
CREATE TABLE IF NOT EXISTS `shopking`.`doklady` (
  ...
  `id_pokladnik` INT(11) NOT NULL,
  `id_zavierka` INT(11) NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_doklady_users1_idx` (`id_pokladnik` ASC) ,
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) ,
  INDEX `fk_doklady_denne_zavierky1_idx` (`id_zavierka` ASC) ,
  CONSTRAINT `fk_doklady_users1`
    FOREIGN KEY (`id_pokladnik`)
    REFERENCES `shopking`.`users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_doklady_denne_zavierky1`
    FOREIGN KEY (`id_zavierka`)
    REFERENCES `shopking`.`denne_zavierky` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `shopking`.`nakupeny_tovar` (
  ...
  `stornovane` TINYINT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
